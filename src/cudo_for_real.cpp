...
double pocSize=0;
	...
	//CvSeq* lines = 0;
	{
		...
		minLINIJA2=3;
		maxRUPA3=30;
		} while (lines->total < 1 && Ocitanja1.size()>minLINIJA2 && glasackiPRAG2>2);
...
//			printf("broj razvrstanih tocaka =%d",velicina);
		...
		if (MAXi != -1)
		{
			CvPoint* line = (CvPoint*)cvGetSeqElem(lines,MAXi);
			Duzina linije=Duzina(Tocka(double(line[0].y)/DISK,double(line[0].x)/DISK)+Xp*(-1),Tocka(double(line[1].y)/DISK,double(line[1].x)/DISK)+Xp*(-1));
...
//					}
		...
		printf("duzina nema dovoljno tocaka (velicina=%d > minLINIJA=%d)",velicina,minLINIJA);
		}					
...
// Za ispis 
	...
	for (i = 0;i<brojac;i++)
	{
		for (int j=0;j<brojac;j++)
			if (i!=j && fitani2[i].A.size()>0 && fitani2[i].B.size()>0)
		{
				
				{
				Duzina AB = fitani2[j].nerazlomljena;
				Duzina CD = fitani2[i].nerazlomljena;
							
				tmp = pojednostavni(&AB,&CD,&ocitanja[0]);
				if (tmp.A.size()>0) //&& tmp.nerazlomljena.duljina() >= AB.duljina() && tmp.nerazlomljena.duljina() >= CD.duljina())				
				{
					
					//*duljina-=fitani2.at(i).A.size()+fitani2.at(j).A.size();;
					//*duljina+=tmp.A.size();
					fitani2[i].B.clear();
					fitani2[i].A.clear();
					fitani2[j]=tmp;
...
// 		for (int i=0;i<Ocitanja1.size();i++)
// 			fitani2[*duljina++]=Razlomljena_Duzina(Duzina(Ocitanja1[i],Ocitanja1[i]));


// Crtanje linija dobivenih linefitom nakon pojednostavljenja	
	//odkom
	/*
	Duzina *rez = new Duzina[*duljina];	
	int brojac2 = 0;
	for (unsigned int i = 0;i<fitani2.size();i++)
	{
		for (unsigned int j = 0;j<fitani2.at(i).A.size();j++)
		{
			rez[brojac2++] = fitani2.at(i).vratiDuzinu(j);
		}
	}
	
	cout << "Preostalo nakon spajanja " << brojac2<< "\n";
	
	//crtaj("Konacni rezultat",brojac2,&rez[0],Xp);
